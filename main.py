...
if st.session_state.page == 'main':
    ...
    with col0:
        with st.popover(""):
            ...
            if st.button("추천 1", use_container_width=1):
                chat("추천 1")
            if st.button("추천 2", use_container_width=1):
                chat("추천 2")
            if st.button("추천 3", use_container_width=1):
                chat("추천 3")
    with col1:
    ...
    with col1:
        ...
        if input_text:
            chat(input_text)
    st.write(f"'{st.session_state.input}' 검색 결과입니다.")
    with st.expander("",expanded=1):
        ytv = st.columns(3)
        for i in range(3):
            with ytv[i]:
